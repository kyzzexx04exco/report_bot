...
1. Sender buka percakapan dengan @notoscam via /start
2. Sender kirim pesan laporan dengan format: @target [deskripsi]
3. Jeda natural antar sender

FIX:
- Tambah resolve @notoscam sebelum send_message agar tidak gagal jika
  sender belum pernah chat dengan notoscam sebelumnya
- Tangani UserPrivacyRestricted, BotBlocked, UserBlocked secara spesifik
- Tambah delay lebih natural antar /start dan isi laporan
"""
...
async def cmd_notoscam(message: Message) -> None:
    ...
    for phone, client in clients:
        try:
            ...
            # Ini penting agar @notoscam menerima pesan berikutnya
            await client.send_message(NOTOSCAM_USERNAME, "/start")
            # Delay natural antara /start dan pesan laporan (seperti user asli ketik)
