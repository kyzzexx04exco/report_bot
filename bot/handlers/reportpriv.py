...
async def resolve_peer_after_join(invite_link: str) -> str | None:
    ...
    FIX:
    - Handle UserAlreadyParticipant: sender sudah di dalam group, cukup get_chat
    - Handle kasus chat.id negatif besar (supergroup) dan positif (basic group)
    - Fallback: jika join_chat tidak return chat object, pakai get_chat manual
    """
    from pyrogram.errors import UserAlreadyParticipant, InviteHashExpired, InviteHashInvalid, ChannelPrivate
    ...
    for s in all_senders:
        ...
        try:
            ...
            try:
                ...
                # Sender sudah member — coba resolve langsung dari link hash
                # Pyrogram bisa get_chat dari invite link di beberapa kasus
                try:
            ...
            if chat:
                # Prioritas 1: username (lebih stabil)
                if getattr(chat, "username", None):
                    logger.info(f"[RESOLVE] {phone} → @{chat.username}")
                    return chat.username
                # Prioritas 2: numeric ID
                if getattr(chat, "id", None):
                    chat_id = chat.id
                    # Supergroup/channel: ID sudah dalam format -100XXXXXXXX
                    # Basic group: ID positif, perlu prefix -
                    if str(chat_id).startswith("-100"):
                        peer_id = str(chat_id)
                    elif chat_id < 0:
                        # Sudah negatif tapi bukan -100 prefix → supergroup lama
                        peer_id = f"-100{abs(chat_id)}"
                    else:
                        # ID positif → basic group
                        peer_id = str(-chat_id)
                    logger.info(f"[RESOLVE] {phone} → {peer_id}")
                    return peer_id

        except (InviteHashExpired, InviteHashInvalid):
            logger.warning(f"[RESOLVE] Link invite expired/invalid: {invite_link}")
            return None  # Link tidak valid, tidak perlu coba sender lain
        except ChannelPrivate:
            logger.warning(f"[RESOLVE] {phone}: channel private, tidak bisa join")
            continue
        except Exception as e:
            logger.warning(f"[RESOLVE] Gagal join/resolve via {phone}: {e}")
            continue
    return None
