...
Report channel/group dengan link PRIVATE (t.me/+ atau t.me/joinchat/).

Logic sama persis dengan reportv2, tapi:
- Menerima link invite private: t.me/+XXXX atau t.me/joinchat/XXXX
- Sender auto-join via invite hash sebelum report
- Eksekutor tetap pakai account.ReportPeer

Flow:
  /reportpriv <invite_link>
...
# Regex untuk private invite link
...
def parse_private_link(text: str):
    """
    Parse link invite private Telegram.
    Return (invite_hash, peer_str) atau None jika tidak valid.
    peer_str untuk private link = link itu sendiri (Pyrogram handle join via hash).
    """
    text = text.strip()
    m = _PRIVATE_INVITE.search(text)
    if m:
        # Kembalikan full link agar Pyrogram bisa join via join_chat(link)
        # dan juga bisa resolve peer setelah join
        # Format yang dikembalikan: full URL agar join_chat bisa tangani
        raw_hash = m.group(1)
...
# ─────────────────────────────────────────────────────────────
# COMMAND: /reportpriv <private_invite_link>
# ─────────────────────────────────────────────────────────────
...
async def cmd_reportpriv(message: Message, state: FSMContext) -> None:
    ...
    # Simpan full_link agar run_report_peer bisa join & resolve peer
    await state.update_data(peer_str=full_link, invite_hash=invite_hash)
...
async def cb_rp_level1(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if cb_data in OPTION_MAP and OPTION_MAP[cb_data][1] is False:
        ...
        peer_str = data.get("peer_str")
        if not peer_str:
            await callback.answer("⚠️ Sesi habis, mulai ulang /reportpriv", show_alert=True)
            await state.clear()
            return
...
async def cb_rp_level2(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if cb_data in RP_L2_TO_KB:
        label_map = {
            "rp_r2_illegalgoods_weapons": "🔫 Weapons",
            "rp_r2_illegalgoods_drugs":   "💊 Drugs",
        }
...
# ─────────────────────────────────────────────────────────────
# CALLBACK: Kirim Tanpa Komentar → pilih pesan
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# MESSAGE: Terima Komentar → pilih pesan
# ─────────────────────────────────────────────────────────────
...
async def handle_rp_comment(message: Message, state: FSMContext) -> None:
    ...
    if message.text and message.text.strip().lower() == "/cancel":
        await state.clear()
        await message.answer("❌ Proses report private dibatalkan.")
        return
...
# ─────────────────────────────────────────────────────────────
# CALLBACK: All Messages → Eksekusi
# ─────────────────────────────────────────────────────────────
