...
# ─────────────────────────────────────────────────────────────
# /settreport <jumlah> — Set berapa kali laporan dikirim per /report
# ─────────────────────────────────────────────────────────────

@router.message(Command("settreport"))
async def cmd_settreport(message: Message) -> None:
    if not is_owner(message.from_user.id):
        await message.answer("⛔ Anda tidak berhak menggunakan bot ini.")
        return
    ...
    if len(parts) < 2:
        # Tampilkan nilai sekarang
        current = await db.get_report_count(message.from_user.id)
        await message.answer(
            f"⚙️ <b>Jumlah Laporan per /report</b>\n\n"
            f"Nilai saat ini: <b>{current}x</b>\n\n"
            f"Artinya setiap kali kamu kirim /report, bot akan otomatis\n"
            f"mengirim laporan sebanyak <b>{current}x</b> memakai akun\n"
            f"sender yang berbeda secara rotasi.\n\n"
            f"Untuk mengubah: <code>/settreport 20</code>",
            parse_mode="HTML",
        )
        return
    ...
    try:
        ...
        await message.answer(
            ...
            "Contoh: <code>/settreport 20</code>",
            parse_mode="HTML",
        )
        return

    if count < 1:
        await message.answer("❌ Jumlah laporan minimal 1.")
        return

    if count > 100:
        await message.answer(
            "❌ Jumlah laporan terlalu besar. Maksimal 100."
        )
        return

    await db.set_report_count(message.from_user.id, count)
    logger.info(
        f"[SETTINGS] Owner {message.from_user.id} set report count ke {count}x."
    )

    await message.answer(
        f"✅ Jumlah laporan diset ke <b>{count}x</b>.\n\n"
        f"Setiap kali kamu pakai /report, bot akan otomatis mengirim\n"
        f"<b>{count}</b> laporan secara rotasi ke semua sender aktif.\n"
        f"Tidak perlu ulang /report berkali-kali!",
        parse_mode="HTML",
    )
