...
FIX:
- is_connected: sebelumnya dipakai sebagai property di temp client,
  padahal client belum pernah connect() — selalu False.
  Sekarang dibungkus try/except untuk aman.
- _cleanup_pending: sebelumnya pakai client.disconnect() yang deprecated,
  sekarang pakai client.stop() dengan fallback ke disconnect().
"""
...
# { admin_user_id: { "client": Client, "phone": str, "phone_code_hash": str } }
...
async def _cleanup_pending(admin_id: int) -> None:
    ...
    if pending and pending.get("client"):
        ...
        try:
            # Pyrogram 2.x: stop() lebih aman dari disconnect()
            await client.stop()
        except Exception:
            try:
...
async def _ensure_temp_connected(client: Client) -> bool:
    """
    Pastikan temp client terkoneksi.
    is_connected mungkin False kalau client belum pernah connect() sebelumnya.
    """
    try:
    ...
    try:
        ...
        logger.error(f"[ADDSENDER] Gagal connect temp client: {e}")
        return False
...
async def handle_otp(message: Message, state: FSMContext) -> None:
    ...
    otp_raw = message.text.strip().replace(" ", "")
    ...
    try:
        # Pastikan client masih terkoneksi
        ok = await _ensure_temp_connected(client)
        if not ok:
            await _cleanup_pending(message.from_user.id)
            await state.clear()
            await message.answer(
                "❌ Koneksi ke Telegram terputus. Mulai ulang dengan /addsender."
            )
            return
