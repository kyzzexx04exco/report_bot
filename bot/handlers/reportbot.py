...
from utils.report_executor import run_report_bot
...
async def cmd_reportbot(message: Message, state: FSMContext) -> None:
    ...
    # Fix #4: parse berbagai format input dengan benar
    peer_str = None

    # Format @username
    if raw.startswith("@"):
        peer_str = raw[1:].strip()
    # Format t.me/username atau https://t.me/username
    elif "t.me/" in raw.lower():
        # Jangan accept invite link t.me/+ untuk bot
        import re
        m = re.search(r"t(?:elegram)?\.me/([A-Za-z0-9_]{3,})", raw, re.IGNORECASE)
        if m:
            peer_str = m.group(1)
    # Format username langsung tanpa @
    elif re.match(r"^[A-Za-z0-9_]{3,}$", raw):
        peer_str = raw

    if not peer_str:
        await message.answer(
            "❌ Username/link tidak valid.\n"
            "Contoh: <code>@somebot</code>, <code>somebot</code>, "
            "atau <code>t.me/somebot</code>",
            parse_mode="HTML",
        )
        return
