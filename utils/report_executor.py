...
# ─────────────────────────────────────────────────────────────
# REPORT PEER — Report langsung channel/group (tanpa message_id)
# Pakai account.ReportPeer (bukan messages.Report)
# ─────────────────────────────────────────────────────────────
...
async def do_report_peer(
    ...
    """
    Invoke account.ReportPeer untuk 1 sender.
    Ini report langsung ke channel/group, bukan pesan spesifik.
    """
    reason_cls = get_reason_cls(option_key)
...
async def run_report_peer(
    ...
    """
    Semua sender aktif nge-report peer langsung (channel/group).
    Auto-join dulu, lalu report dengan account.ReportPeer.
    """
    all_senders = await db.get_all_senders()
    active_senders = [s for s in all_senders if s.get("is_active")]
    ...
    # Auto-join semua sender dulu
    logger.info(f"[REPORT-PEER] Auto-join {len(active_senders)} sender ke {peer_str}...")
    await join_all_senders(peer_str)
    ...
    for sender in active_senders:
        ...
        if client is None:
            logger.warning(f"[REPORT-PEER] {phone} tidak di pool, skip.")
            failed_senders.append(phone)
            continue
        ...
        for i in range(repeat):
            try:
                ...
                total_sent += 1
                logger.info(
                    f"[REPORT-PEER] ✅ {phone} — {i+1}/{repeat} "
                    f"(total: {total_sent})"
                )
                if i < repeat - 1:
                    await asyncio.sleep(0.5)
                ...
                logger.warning(
                    f"[REPORT-PEER] ⚠️ FloodWait {fw.value}s pada {phone} "
                    f"setelah {sender_sent} report. Skip sender ini."
                )
                await db.set_sender_inactive(phone)
                await pyro_client.remove_client_from_pool(phone)
                ...
                logger.error(f"[REPORT-PEER] Peer error: {e}")
                return {
                    "success": False,
                    "message": f"❌ {e}",
                    "total_sent": total_sent,
                ...
                logger.error(f"[REPORT-PEER] Error {phone}: {e}")
                break
