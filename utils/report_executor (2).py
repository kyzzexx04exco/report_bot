...
Untuk reportbot:
  1. account.UpdateStatus(offline=False)        → set online
  2. messages.StartBot                          → sender ketuk /start di bot
  3. account.ReportPeer(peer, reason)           → report user/bot
...
# ─────────────────────────────────────────────────────────────
# STEP 1 — SET ONLINE
# account.UpdateStatus(offline=False)
# ─────────────────────────────────────────────────────────────
...
# messages.GetHistory → ambil IDs
# channels.ReadHistory (channel) / messages.ReadHistory (group)
# → mark as read. INI TIDAK NAMBAH VIEW, tapi perlu untuk
#   simulasi natural seperti user buka channel beneran.
# ─────────────────────────────────────────────────────────────
...
async def _fetch_and_read(
    ...
    try:
        ...
        # Coba channels.ReadHistory dulu (channel/supergroup)
        # fallback ke messages.ReadHistory (basic group/user dialog)
        try:
            await client.invoke(
...
# messages.GetMessagesViews(increment=True)
#
# Dari official Telegram docs (core.telegram.org/api/views):
# "to increment the view counter and fetch the updated view counter,
#  forward counter and thread information, invoke the
#  messages.getMessagesViews method with increment set to boolTrue"
#
# Ini SATU-SATUNYA method yang beneran nambah view counter.
# Max 100 IDs per request, max ~2x per hari per akun.
# ─────────────────────────────────────────────────────────────
...
# messages.StartBot → sender ketuk /start di bot target
# persis seperti user buka bot baru dan klik tombol Start
# ─────────────────────────────────────────────────────────────

async def _start_bot(client: Client, peer_str: str, phone: str) -> None:
    try:
        await client.start_bot(peer_str)
        logger.info(f"[STARTBOT] ✅ {phone} start bot @{peer_str}")
        await asyncio.sleep(_rnd(1.5, 3.0))
    except Exception as e:
        # Sudah pernah start / bukan bot — non-fatal, lanjut report
        logger.warning(f"[STARTBOT] {phone} gagal (non-fatal, lanjut report): {e}")


# ─────────────────────────────────────────────────────────────
# STEP 6 — REPORT
# messages.Report → dengan IDs (efektif)
# account.ReportPeer → fallback tanpa IDs
# ─────────────────────────────────────────────────────────────
...
async def run_report_bot(
    ...
    """/reportbot — start bot dulu, lalu report user/bot."""
    return await _run_engine(
        peer_str=peer_str, option_key=option_key, comment=comment,
        admin_user_id=admin_user_id, repeat=repeat,
        seed_message_ids=[],
