...
- Cek koneksi pakai get_me() bukan is_connected (property private)
- Tambah delay setelah reconnect agar peer cache terisi dulu
- Tangani peer cache miss (KeyError 'ID not found') dengan graceful
"""
...
def create_client_from_session(phone_number: str, session_string: str) -> Client:
    ...
    return Client(
        ...
        in_memory=True,
        no_updates=False,   # False agar sender keliatan online & bisa terima update
        workdir="/tmp",
    )
...
async def _is_alive(client: Client) -> bool:
    ...
    Pakai get_me() — lebih ringan dan reliable daripada is_connected.
    """
    try:
        await client.get_me()
        return True
...
async def load_all_sessions(senders: list[dict]) -> None:
    ...
    for sender in senders:
        ...
        try:
            ...
            # Delay kecil setelah start agar peer cache Pyrogram mulai terisi
...
async def ensure_connected(phone: str, session_string: str) -> Optional[Client]:
    ...
    FIX: Tambah delay setelah reconnect agar Pyrogram peer-cache sempat
    terisi sebelum dipakai — tanpa ini akan muncul:
      KeyError: 'ID not found: <peer_id>'
    karena internal storage Pyrogram belum resolve peer tersebut.
    """
    ...
    try:
        ...
        # ── FIX: delay 2 detik setelah start ────────────────────────────────
        # Pyrogram butuh waktu untuk resolve & cache peer dari session lama.
        # Tanpa ini → KeyError 'ID not found: <peer_id>' saat resolve_peer.
        ...
        logger.error(f"[RECONNECT] ❌ {phone} session invalid: {e}")
        from database import db
        await db.set_sender_inactive(phone)
        return None
