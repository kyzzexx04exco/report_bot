...
async def _safe_start(client) -> bool:
    """
    Wrapper start() yang aman — Pyrogram 2.0.106 kadang raise AttributeError
    internal ('NoneType' has no attribute 'close') waktu proxy MTProto
    gagal konek. Kita catch semua exception termasuk BaseException.
    ...
    try:
        ...
        raise  # re-raise biar ditangani di caller
    except BaseException as e:
        logger.error(f"[POOL] start() gagal (internal Pyrogram): {e}")
        return False
...
async def load_all_sessions(senders: list[dict]) -> None:
    ...
    for sender in senders:
        ...
        try:
            ...
            if not ok:
                logger.error(f"[POOL] ❌ Gagal load {phone}: start() gagal, skip.")
                continue
